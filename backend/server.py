...
# Temporary preview endpoint for design-mockup PNGs generated by
# scripts/gen_login_mockup.py. Lives outside /app/backend/generated/.
# No auth — meant to be opened in the user's browser while iterating
# on the visual direction. Returns the most-recent mockup by mtime.
_MOCKUP_DIR = Path("/app/backend/generated")


@app.get("/api/preview/login-mockup")
async def preview_login_mockup():
    if not _MOCKUP_DIR.exists():
        raise HTTPException(404, "No mockups generated yet")
    pngs = sorted(_MOCKUP_DIR.glob("*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not pngs:
        raise HTTPException(404, "No mockups generated yet")
    return FileResponse(
        path=str(pngs[0]),
        ...
        headers={
            ...
            "Content-Disposition": f'inline; filename="{pngs[0].name}"',
        },
    )
